...
#[cfg_attr(
    feature = "clap",
    clap(next_help_heading = "TRACING", term_width = 200)
)]
pub struct TracingArgs {
    #[cfg_attr(feature = "env", env(rename = "HUMAN_READABLE_LOGS", default))]
    #[cfg_attr(
        feature = "clap",
        clap(name = "human-readable-logs", env = "HUMAN_READABLE_LOGS", long)
    )]
    ...
    pub otel: Option<crate::otel::OpentelemetryArgs>,
}
...
impl TracingArgs {
        ...
        Sink: for<'a> tracing_subscriber::fmt::MakeWriter<'a> + Send + Sync + 'static + Clone,
    {
        ...
        let subscriber = ground_commons::tracing::make_subscriber(default_env_filter, json, sink)?;
        ...
        #[cfg(feature = "cli-otel")]
        {
            ...
            let tracer = self
                .otel
                .as_ref()
                .and_then(|args| args.to_tracer(service_name).transpose());
            if let Some(tracer) = tracer {
                let layer = tracing_opentelemetry::layer().with_tracer(tracer?);
                use tracing_subscriber::layer::SubscriberExt;
                return ground_commons::tracing::set_global_default(subscriber.with(layer));
            }
        }

        ground_commons::tracing::set_global_default(subscriber)
    }
}
