...
// @TODO jezza - 20 Sept 2024: Add support for inline modules.
pub struct Module {
    pub name: String,
...
impl Module {
    ...
    pub fn write(&self, target: impl AsRef<Path>) -> anyhow::Result<()> {
        let target = target.as_ref();
        let module_path = self.write_inner(target)?;

        // @TODO jezza - 20 Sept 2024: Add a way to disable this.
    ...
    fn write_inner(&self, target: &Path) -> anyhow::Result<PathBuf> {
        std::fs::create_dir_all(target).context("Unable to create directory.")?;

        // Don't return early, so the developer can figure out what was generated incorrectly.
        let file: Result<syn::File, _> = syn::parse2(self.content.clone()).with_context(|| {
            let name = self.all_segments().join("::");
            format!("Module {} contained invalid rust syntax", name)
        });

        let (content, parse_err): (TokenStream, Option<anyhow::Error>) = match file {
            Ok(mut file) => {
                ...
                // @TODO jezza - 20 Sept 2024: Should we only sort `use` up to the first non-`use` item?
        ...
        write(content, &module_path).context("Unable to write file.")?;
        ...
        for module in self.children.iter() {
            module
                .write_inner(&target)
                .context("Unable to write child module")?;
        }
...
fn write(tokens: TokenStream, path: &Path) -> anyhow::Result<()> {
    ...
    #[cfg(feature = "pretty")]
    fn format(content: String) -> anyhow::Result<String> {
