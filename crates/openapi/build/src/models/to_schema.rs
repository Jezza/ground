    ...
    fn convert(&self) -> schemars::schema::Schema {
        match self {
            openapiv3::ReferenceOr::Reference { reference } => {
                // @HACK jezza - 07 Oct 2024: This is just a temporary solution until they expose more within typify.
                let reference = reference
                    .rfind('/')
                    .and_then(|pos| {
                        let path = &reference[0..pos];
                        let full_name = &reference[pos + 1..];

                        full_name.rfind('.').map(|pos| {
                            let partial_name = &full_name[pos + 1..];
                            format!("{}/{}", path, partial_name)
                        })
                    })
                    .unwrap_or_else(|| reference.clone());

                schemars::schema::SchemaObject::new_ref(reference).into()
            }
